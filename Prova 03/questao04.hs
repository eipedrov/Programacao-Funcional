...
comparar n1 n2
    | length (show n1) >= length (show n2) = do 
        if compareWith (show n1) (show n2) 
            then "encaixa" 
            else let n1tail=read(drop 1 (show n1))::Int in comparar n1tail n2
...
main = do
    a <- readLn :: IO Int
    b <- readLn :: IO Int
    putStrLn(comparar a b)

--Input: main 
-- os dois valores a serem comparados
--Saida: 
-- se encaixa ou não 